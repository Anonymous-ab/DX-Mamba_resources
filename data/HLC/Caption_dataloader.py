...
class Caption_Dataset(Dataset):
    ...
    def __init__(self, encoder_network, data_folder, caption_path, token_folder = None, word_vocab = None, max_length = 240, allow_unk = 0, max_iters=None):
        """
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.data.iloc[idx, 4].split()
        ...
        if 'CLIP' not in self.encoder_network:
            imgA = Image.fromarray(imread(image_path1)).resize((224, 224))
            imgB = Image.fromarray(imread(image_path2)).resize((224, 224))
            # print('xx', imgA.shape)
...
#     parser.add_argument('--max_length', type=int, default=240, help='path of the data lists')
